...
🛡️ **Anti-Nuke** — Protect from nukers, raiders & ghost pings
⭐ **Premium** — Giveaways, Levels, Reaction Roles & more
🚫 **Anti-Spam** — Auto-timeout spammers
🔐 **Security** — Server security audit tools
📢 **Ticket System** — Support ticket management
🛡️ **Moderation** — Ban, kick, warn, timeout
📢 **Invite Logging** — Track who invited who
⏰ **Timer** — Set timers
💤 **AFK** — AFK status system
🔧 **Utility** — Userinfo, avatar, ping
🛠️ **Logs** — Set ticket & bot log channels
🛡️ **Verification** — Member verification system
👋 **Welcome** — Join, leave & boost messages
...
async def send_log(guild, kind, title, description, color=discord.Color.from_rgb(155, 89, 182)):
    c = cfg(guild.id)
...
def is_mod():
    async def predicate(ctx):
        if not ctx.guild or not isinstance(ctx.author, discord.Member):
            return False
        return ctx.author.guild_permissions.manage_guild or ctx.author.guild_permissions.manage_messages or (ctx.author.id == MAIN_ADMIN_ID or ctx.author.guild_permissions.administrator)
    return commands.check(predicate)


def is_admin():
    async def predicate(ctx):
        return bool(ctx.guild and isinstance(ctx.author, discord.Member) and (ctx.author.id == MAIN_ADMIN_ID or ctx.author.guild_permissions.administrator))
    return commands.check(predicate)
...
class HelpSelect(discord.ui.Select):
    def __init__(self, owner_id=None):
        opts = [discord.SelectOption(label=x, description=CATEGORY_DESC[x][:100], emoji=CATEGORY_ICONS.get(x, "📁")) for x in CATEGORY_ORDER]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.owner_id and interaction.user.id != self.owner_id:
            ...
            return
        embed = discord.Embed(description=CATEGORY_TEXT[category], color=discord.Color.from_rgb(155, 89, 182))
    ...
    try:
        await bot.change_presence(
            ...
            activity=discord.Streaming(
                ...
                url="https://www.twitch.tv/notryxenyt"
            )
        )
...
async def help_command(ctx):
    embed = discord.Embed(description=CATEGORY_TEXT["Index"], color=discord.Color.from_rgb(155, 89, 182))
    embed.set_author(name=BOT_NAME)
    embed.set_footer(text=FOOTER)
    file = discord.File(LOGO_FILE, filename="logo.png") if os.path.exists(LOGO_FILE) else None
    embed.set_thumbnail(url="attachment://logo.png" if file else bot.user.display_avatar.url)
...
async def botinfo(ctx):
    e = discord.Embed(title=f"{BOT_NAME}", description="Feature-rich multipurpose Discord bot.", color=discord.Color.from_rgb(155, 89, 182))
...
async def userinfo(ctx, member: discord.Member=None):
    ...
    e=discord.Embed(title=str(member), color=member.color if member.color != discord.Color.default() else discord.Color.from_rgb(155, 89, 182))
...
async def serverinfo(ctx):
    g=ctx.guild
    e=discord.Embed(title=g.name, color=discord.Color.from_rgb(155, 89, 182))
...
async def announce(ctx, channel: discord.TextChannel, *, message):
    e=discord.Embed(title="📢 Announcement", description=message, color=discord.Color.from_rgb(155, 89, 182), timestamp=datetime.now(timezone.utc))
    e.set_footer(text=FOOTER)
    await channel.send(embed=e)
...
async def link(ctx, channel: discord.TextChannel, url: str, *, text="Open Link"):
    ...
    e=discord.Embed(title="🔗 Link", description=f"[{text}]({url})", color=discord.Color.from_rgb(155, 89, 182))
    await channel.send(embed=e)
...
async def welcome_test(ctx):
    ...
    await ctx.send(embed=discord.Embed(title="👋 Welcome Preview", description=msg, color=discord.Color.from_rgb(155, 89, 182)))
...
async def ticket_panel(ctx):
    e=discord.Embed(title="🎫 Support Tickets", description="Click **Open Ticket** to create a private support channel.", color=discord.Color.from_rgb(155, 89, 182))
...
async def poll(ctx, duration: str, *, content):
    ...
    e=discord.Embed(title="📊 Poll",description=f"**{question}**\n\n"+"\n".join(f"{i+1}️⃣ {o}" for i,o in enumerate(options)),color=discord.Color.from_rgb(155, 89, 182))
    msg=await ctx.send(embed=e)
...
for _name, _category in [("premium","Premium"),("giveaways","Giveaways"),("timer","Timer"),("utility","Utility")]:
    async def _cat(ctx, _cat=_category):
        await ctx.send(embed=discord.Embed(description=CATEGORY_TEXT[_cat], color=discord.Color.from_rgb(155, 89, 182)), view=HelpView(ctx.author.id))
    bot.command(name=_name)(_cat)

# Graceful start
if not TOKEN or TOKEN == "PASTE_YOUR_DISCORD_BOT_TOKEN_HERE":
    print("ERROR: Paste your Discord bot token into TOKEN in bot.py")
else:
    bot.run(TOKEN)
