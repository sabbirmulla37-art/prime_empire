...
def admin_only(interaction: discord.Interaction):
    return interaction.user.id == MAIN_ADMIN_ID or interaction.user.guild_permissions.administrator
...
async def antinuke_status(ctx):
    c = cfg(ctx.guild.id)
    log_value = f"#{c['antinuke_log']}" if c.get('antinuke_log') else 'not set'
    await ctx.send(
        f"Anti-Nuke: `{bool(c.get('antinuke_enabled'))}`\n"
        f"Raid shield: `{bool(c.get('raid_enabled'))}`\n"
        f"Log: {log_value}"
    )
...
# IMPORTANT: Do NOT upload a filled-in token to GitHub.
# ============================================================
